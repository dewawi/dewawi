...
class Purchases_PositionController extends Zend_Controller_Action
{
	...
	public function indexAction()
	{
		...
		foreach($positionSets as $positionSet) {
			...
			foreach($positions as $position) {
				if($positionSet->id == $position->possetid) {
					...
					$forms[$position->id]->ordering->addMultiOptions($this->_helper->Ordering->getOrdering($params['parent'], $params['parentid'], $positionSet->id));
		...
		$parent['subtotal'] = $currency->toCurrency($parent['subtotal']);
		$parent['total'] = $currency->toCurrency($parent['total']);
	...
	public function applyAction()
	{
		...
		if($request->isPost()) {
			...
			if($params['itemid'] && $params['parentid']) {
				...
				$data['ordering'] = $this->_helper->Ordering->getLatestOrdering($params['parent'], $params['parentid'], $params['setid']) + 1;
	...
	public function editAction()
	{
		...
		//Define belonging classes
		$formClass = 'Purchases_Form_'.ucfirst($params['parent'].'pos');
		$modelClass = 'Purchases_Model_DbTable_'.ucfirst($params['parent'].'pos');

		$form = new $formClass();
		$form->uom->addMultiOptions($uoms);
		$form->ordering->addMultiOptions($this->_helper->Ordering->getOrdering($params['parent'], $params['parentid'], 0));
	...
	public function copyAction()
	{
		...
		if($request->isPost()) {
			...
			$this->_helper->Ordering->pushOrdering($data['ordering'], $params['parent'], $data['parentid'], $data['possetid']);
			$data['ordering'] += 1;
			$data['modified'] = NULL;
			$data['modifiedby'] = 0;
			unset($data['id']);
	...
	public function sortAction()
	{
		...
		if($request->isPost()) {
			$data = $request->getPost();
			$this->_helper->Ordering->sortOrdering($data['id'], $params['parent'], $params['parentid'], $params['setid'], $data['ordering']);
		}
	}
	...
	public function deleteAction()
	{
		...
		if($request->isPost()) {
			...
			if($data['delete'] == 'Yes') {
				...
				$this->_helper->Ordering->setOrdering($params['parent'], $params['parentid'], $params['setid']);
