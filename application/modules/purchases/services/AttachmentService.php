...
class Purchases_Service_AttachmentService
{
	public function sync(array $document, array $contact, string $controller): array
	{
		...
		if ($filename === '') {
			return [
				...
				'attachments' => $emailattachmentDb->getEmailattachments($documentId, 'purchases', $controller),
			];
		}

		$sourceFile = BASE_PATH . '/files/contacts/' . $contactUrl . '/' . $filename;
		$targetDir = BASE_PATH . '/files/attachments/purchases/' . $controller . '/' . $documentUrl;
