...
class Items_Model_DbTable_Itemlist extends DEEC_Model_DbTable_Entity
{

	protected $_name = 'itemlist';
	...
	public function getItemlistsByCategory($catid)
	{
		$catid = (int)$catid;
	...
	public function getLatestItemlists()
	{
		$where = array();
		$where[] = $this->getAdapter()->quoteInto('clientid = ?', $this->_client['id']);
		$where[] = $this->getAdapter()->quoteInto('deleted = ?', 0);
	...
	public function updateItemlist($id, $data)
	{
		$id = (int)$id;
	...
	public function quantityItemlist($id, $quantity)
	{
		$id = (int)$id;
		$data = array();
