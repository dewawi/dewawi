...
class Items_Model_DbTable_Inventory extends DEEC_Model_DbTable_Entity
{
	...
	public function getInventoryForEdit($id)
	{
		$id = (int)$id;
	...
	public function getInventories($ids)
	{
	...
	public function getInventoriesByCategory($catid)
	{
		$catid = (int)$catid;
		$where = array();
	...
	public function getLatestInventories()
	{
		$where = array();
		$where[] = $this->getAdapter()->quoteInto('clientid = ?', $this->_client['id']);
		$where[] = $this->getAdapter()->quoteInto('deleted = ?', 0);
	...
	public function quantityInventory($id, $quantity)
	{
		$id = (int)$id;
		$data = array();
