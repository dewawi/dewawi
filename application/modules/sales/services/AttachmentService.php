...
class Sales_Service_AttachmentService
{
	public function sync(array $document, array $contact, string $controller): array
	{
		...
		if ($filename === '') {
			return [
				...
				'attachments' => $emailattachmentDb->getEmailattachments($documentId, 'sales', $controller),
			];
		}

		$sourceFile = BASE_PATH . '/files/contacts/' . $contactUrl . '/' . $filename;
		$targetDir = BASE_PATH . '/files/attachments/sales/' . $controller . '/' . $documentUrl;
