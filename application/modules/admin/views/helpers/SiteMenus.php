...
class Zend_View_Helper_SiteMenus extends Zend_View_Helper_Abstract
{
	public function SiteMenus(array $config = [])
	{
		...
		$items = $db->getMenusByShopId($shopId);
		...
		$list = new Admin_Model_List_Menus();
		$list->configure([
			'id' => 'site-menus',
			'items' => $items,
			'view' => $this->view,
		...
		ob_start();
		?>
		<div class="dw-child-list" data-list-id="site-menus" data-controller="menu" data-shopid="<?php echo $this->view->escape($shopId); ?>">
			<div class="dw-child-list-toolbar">
				<a class="dw-btn" href="<?php echo $this->view->url([
					...
					<?php echo $this->view->translate('ADMIN_NEW_MENU'); ?>
				</a>
			</div>
