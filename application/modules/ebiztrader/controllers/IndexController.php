...
class Ebiztrader_IndexController extends Zend_Controller_Action
{
	...
	public function indexAction()
	{
		...
		$form = new Ebiztrader_Form_Listing();
		$toolbar = new Ebiztrader_Form_Toolbar();
	...
	public function syncAction()
	{
		...
		if($accountid) {
			$accountDb = new Ebiztrader_Model_DbTable_Account();
			...
			if($account) {
				...
				require_once(BASE_PATH.'/library/Dewawi/ebizTrader.php');
				$ebizTrader = new Dewawi_ebizTrader(BASE_PATH, DB_SERVER, DB_USER, DB_PASSWORD, DB_NAME);
				$ebizTrader->listItems($accountid);
	...
	public function addAction()
	{
		...
		if($request->isPost()) {
			...
			if($form->isValid($data)) {
				$listingDb = new Ebiztrader_Model_DbTable_Listing();
	...
	public function editAction()
	{
		...
		$itemDb = new Ebiztrader_Model_DbTable_Item();
		...
		if(false) {
			...
			if($request->isPost()) {
				...
				if($id > 0) {
					...
					//Attributes
					$attributesDb = new Ebiztrader_Model_DbTable_Itemattribute();
					$attributes = $attributesDb->getItemattributesByItemID($item['id']);

					//History
					$inventoryDb = new Ebiztrader_Model_DbTable_Inventory();
	...
	public function copyAction()
	{
		...
		$id = $this->_getParam('id', 0);
		$item = new Ebiztrader_Model_DbTable_Item();
